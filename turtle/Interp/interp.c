...
int main(int argc, char* argv[]){
  ...
  file_to_prog(prog, argv[1]);
  if(argc == 2){
    neillclrscrn();
  }
  if(argc == 3 && strstr(argv[2], ".ps") != NULL){
    ps_file_initial(prog, argv[2]);
  }
  Prog(prog);
  if(prog->fp != NULL){
      fputs("showpage", prog->fp);
  }
  if(argc == 2){
    neillreset();
  }
  if(argc == 3 && strstr(argv[2], ".txt") != NULL){
    result_print(prog, argv[2]);
  }
  if(argc == 3 && strstr(argv[2], ".ps") != NULL){
    fclose(prog->fp);
    char command[100] = "ps2pdf ";
...
void command_test(int argc, char** argv){
  if(argc != 2 && argc != 3){
    fprintf(stderr, "Retry it using: 1. %s <argv1> 2. %s <argv1> <argv2>. <argv1> is input file and <argv2> is output .txt/.ps file\n", argv[0], argv[0]);
    exit(EXIT_FAILURE);
  }

  if(argc == 3 && strstr(argv[2], ".txt") == NULL && strstr(argv[2], ".ps") == NULL){
    fprintf(stderr, "Retry it using: 1. %s <argv1> 2. %s <argv1> <argv2>. <argv1> is input file and <argv2> is output .txt/.ps file\n", argv[0], argv[0]);
    exit(EXIT_FAILURE);
  }
}
